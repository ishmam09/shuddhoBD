...
Where:
* $R_u$ is the count of unresolved citizen complaints verified in the constituency.
* $P_d$ is the project delay indicator measured in months.
* $G_p$ is the verified number of "ghost projects" (funded initiatives showing zero physical presence on-site).
...
* **Frontend Web Client:** Runs client-side sorting and search query structures. Renders data charts via Chart.js and loads the interactive map coordinate arrays through Google Maps.
